...
async fn handle_connection(stream: UnixStream, client: Arc<EmbeddedClient>) -> std::io::Result<()> {
...
mod tests {
    ...
    async fn hello_over_the_socket_matches_the_embedded_path() {
        ...
        let sock = std::env::temp_dir().join(format!("valyria-test-{}.sock", std::process::id()));
    ...
    async fn task_lifecycle_drives_over_the_socket() {
        let rt = test_runtime().await;
        let sock = std::env::temp_dir().join(format!("valyria-test-{}-b.sock", std::process::id()));
        let shutdown = CancellationToken::new();
        let server = tokio::spawn(serve(rt.clone(), sock.clone(), shutdown.clone()));
        for _ in 0..100 {
            if sock.exists() {
    ...
    async fn events_stream_over_the_socket_reaches_a_terminal_state() {
        let rt = test_runtime().await;
        let sock = std::env::temp_dir().join(format!("valyria-test-{}-c.sock", std::process::id()));
        let shutdown = CancellationToken::new();
        let server = tokio::spawn(serve(rt.clone(), sock.clone(), shutdown.clone()));
        for _ in 0..100 {
            if sock.exists() {
